"use strict";
// Programmatic API — wraps the driftabot binary and returns parsed results.
...
/**
 * Run the driftabot binary and return its stdout.
 * @param {string[]} args
 * @returns {string}
 */
function run(args) {
  ...
  if (result.error) {
    throw new Error(`driftabot binary error: ${result.error.message}`);
  }
  if (result.status !== 0 && result.status !== 1) {
    // exit 1 means breaking changes found (--fail-on-breaking), not a crash
    throw new Error(`driftabot exited with code ${result.status}: ${result.stderr}`);
  }
...
function impact(diffResult, scanDir = ".", options = {}) {
  ...
  const tmp = require("path").join(os.tmpdir(), `driftabot-diff-${Date.now()}.json`);
  try {
