...
```sh
docker build -t driftabot-engine .
docker run -p 50051:50051 driftabot-engine
```

Override the port via the `PORT` environment variable:

```sh
docker run -e PORT=9090 -p 9090:9090 driftabot-engine
```
...
The proto definition lives at [`api/drift-agent/v1/drift-agent.proto`](https://github.com/DriftaBot/driftabot-engine/blob/main/api/drift-agent/v1/drift-agent.proto).
